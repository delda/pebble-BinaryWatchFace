#!/usr/bin/env bash
# Capture the documented Gabbro colour watchface configurations into a GIF.
set -euo pipefail

project_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
platform="${1:-gabbro}"
output_dir="${2:-$project_dir/screenshots/configurations-gabbro}"
...
if [[ "$platform" != "gabbro" ]]; then
  echo "This capture set targets the Gabbro emulator." >&2
  exit 2
fi
