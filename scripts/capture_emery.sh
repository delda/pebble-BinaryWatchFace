#!/usr/bin/env bash
# Capture the documented Emery colour watchface configurations into a GIF.
set -euo pipefail

project_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
platform="${1:-emery}"
output_dir="${2:-$project_dir/screenshots/configurations-emery}"
...
if [[ "$platform" != "emery" ]]; then
  echo "This capture set targets the Emery emulator." >&2
  exit 2
fi
